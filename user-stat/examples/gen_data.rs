...
const DB_URL: &str = "postgres://kindy:kindy@localhost:5432/stats";

/*
select pg_size_pretty(pg_database_size('stats')) as size;

select pg_size_pretty(pg_relation_size('user_stats')) as size;

select pg_size_pretty(pg_relation_size('user_stats_pkey')) as size;

select pg_size_pretty(pg_relation_size('user_stats_recent_watched_idx')) as size;
 */
...
fn gen_fake_user_stats(max: u32) -> HashSet<UserStat> {
    let user_stats: HashSet<_> = (0..max).map(|_| Faker.fake::<UserStat>()).collect();
    user_stats
}
#[allow(dead_code)]
async fn bulk_insert(user_stats: HashSet<UserStat>, pool: &PgPool) -> Result<()> {
    let mut tx = pool.begin().await?;
...
#[allow(dead_code)]
async fn raw_insert(user_stats: HashSet<UserStat>, pool: &PgPool) -> Result<()> {
...
#[allow(dead_code)]
async fn raw_insert_tran(
    user_stats: HashSet<UserStat>,
